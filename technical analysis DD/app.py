...
def load_crypto_data(crypto_symbol):
    ...
    try:
        ...
        if crypto_symbol in ['AVAX_USD', 'PI_USD', 'TRX_USD']:
            filename = f"./csvdata/{dropdown_dict[crypto_symbol]} Historical Data.csv"
        else:
            # For regular cryptocurrency files
            filename = f"./csvdata/{dropdown_dict[crypto_symbol]}_{crypto_symbol}_historical_data.csv"
...
def update_rsi_chart(crypto_symbol):
    df = load_crypto_data(crypto_symbol)
    if df is None:
        return go.Figure()
...
def update_macd_chart(crypto_symbol):
    df = load_crypto_data(crypto_symbol)
    if df is None:
        return go.Figure()
...
def update_atr_chart(crypto_symbol):
    df = load_crypto_data(crypto_symbol)
    if df is None:
        return go.Figure()
    return create_atr_chart(df)
